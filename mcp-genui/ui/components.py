"""Prefab UI component builders — tool result cards + reasoning card.
...
# ── NEW: Reasoning card for v2 agent ──

def build_reasoning_card(reasoning_text: str) -> Column:
    """Render agent reasoning step as a Prefab card with labeled badges.

    Parses [THINK], [VERIFY], [FALLBACK], [ACT], [REASON_TYPE] labels
    from the reasoning text and renders each as a Badge + text row.
    """
    labels_config = {
        "THINK": "default",
    ...
    parsed = []
    for label, variant in labels_config.items():
        pattern = rf"\[{label}\]\s*(.+?)(?=\[(?:THINK|REASON_TYPE|VERIFY|FALLBACK|ACT)\]|$)"
        match = _re.search(pattern, reasoning_text, _re.DOTALL)
        if match:
            content = match.group(1).strip()
            if content:
                parsed.append((label, content, variant))

    # Extract REASON_TYPE value for header badge
    reason_type = ""
    for label, content, _ in parsed:
        if label == "REASON_TYPE":
            reason_type = content
            break

    with Column(gap=2) as layout:
        with Card():
            with CardHeader():
                with Row(gap=1):
                    Badge("Reasoning", variant="secondary")
                    if reason_type:
                        Badge(reason_type, variant="outline")

            with CardContent():
                with Column(gap=1):
                    for label, content, variant in parsed:
                        if label == "REASON_TYPE":
...
def build_dashboard(results: list[dict]) -> Column:
    ...
    Special entries: {"tool": "_answer", ...} and {"tool": "_reasoning", ...}
    """
    sections = []
    for entry in results:
        ...
        try:
            ...
            elif tool_name in ("generate_prefab_ui", "search_prefab_components"):
                continue  # rendered client-side via Pyodide

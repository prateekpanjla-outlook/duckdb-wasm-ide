"""Agent harness v2 — Gemini + MCP client with ReACT reasoning.

Same architecture as v1 but with:
  - Enhanced system prompt (reasoning framework: THINK/VERIFY/FALLBACK/ACT/REASON_TYPE)
  - Reasoning text yielded as separate SSE events before tool calls
  - Passes 9/9 on the prompt evaluation rubric
...
Flow:
  ...
  4. Gemini returns TEXT (reasoning) + functionCall(s) as separate parts
  5. Yield reasoning as {"type": "reasoning"} event
  6. Execute tool via MCP, yield tool_call + tool_result events
...
# Add parent to path for v2 config
...
# Import v2 MCP server instance for in-memory client connection
from v2.mcp_server_v2 import mcp_v2 as mcp_server

# Tools the agent should NOT send to Gemini (UI-only, not for LLM)
EXCLUDED_TOOLS = {"render_dashboard"}

# ── System Prompt v2 (v1 prompt + ReACT reasoning framework) ──────────
...
[REASON_TYPE] — Classify: concept_selection | schema_design | data_generation | query_logic | verification | error_recovery
...
# ── Agent loop (v2 — with reasoning extraction) ───────────────────

async def run_agent_v2(prompt: str, api_key: str = None, model: str = None):
    """Run the full agent loop with ReACT reasoning. Yields step dicts for SSE streaming.
    ...
    print("=" * 62)
    print(f"[AGENT v2] Starting — prompt: \"{prompt}\"")
    print(f"[AGENT v2] Model: {model} | Max steps: {MAX_STEPS}")
    print("=" * 62)

    print("\n[MCP] Connecting to MCP server v2 (in-memory transport)...")
    ...
    async with Client(mcp_server) as client:
        ...
        while step_count < MAX_STEPS:
            ...
            # ── v2: Emit reasoning text BEFORE tool calls ──
        ...
        print(f"\n{'=' * 62}")
        print(f"[AGENT v2] Complete: {step_count} steps, {tool_calls_made} tool calls")
        print(f"[AGENT v2] Tools used:      {', '.join(sorted(tools_used)) or '(none)'}")
        if unused:
            print(f"[AGENT v2] Tools available:  {', '.join(sorted(unused))} (not needed)")
        print(f"{'=' * 62}")


async def main():
    parser = argparse.ArgumentParser(description="MCP Question Authoring Agent v2 (ReACT)")
    ...
    print(f"\nAgent v2: model={model}, prompt={args.prompt}\n")

    async for step in run_agent_v2(args.prompt, api_key, model):

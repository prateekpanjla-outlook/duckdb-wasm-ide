...
# ── v2 Routes — ReACT reasoning agent ──

@app.get("/v2", response_class=HTMLResponse)
async def index_v2():
    """Serve the v2 landing page with reasoning UI."""
    return HTMLResponse(LANDING_PAGE_V2)


@app.post("/agent/v2/stream")
async def agent_stream_v2(request: Request):
    """SSE endpoint — runs the v2 agent loop with reasoning."""
    body = await request.json()
    prompt = body.get("prompt", "")
    admin_key = body.get("admin_key", "")
    ...
    async def event_generator():
        try:
            async for step in run_agent_v2(prompt):
                yield f"data: {json.dumps(step)}\n\n"
            yield f"data: {json.dumps({'type': 'done'})}\n\n"
        except Exception as e:
            yield f"data: {json.dumps({'type': 'error', 'content': str(e)})}\n\n"
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SQL Practice — Question Authoring Agent v2 (ReACT)</title>
    <script type="importmap">{"imports":{"zod/v4":"/js/zod-v4.js"}}</script>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
        ...
        .header .version-badge {
            font-size: 10px; background: #7c3aed; color: white;
...
<body>
    <div class="header">
        <h1>MCP Agent</h1>
        <span class="version-badge">v2 ReACT</span>
        <input type="password" id="adminKey" placeholder="Admin Key">
        <input type="text" id="prompt" placeholder="e.g. Add a question about INNER JOIN">
        <button id="runBtn">Run Agent</button>
    </div>
    ...
    <script type="module">
        try {
            ...
            const bridge = new AppBridge(
                client,
                { name: "mcp-agent-v2", version: "2.0.0" },
                {
                    openLinks: {},
                    serverTools: serverCaps?.tools,
                    serverResources: serverCaps?.resources,
            ...
            bridge.onmessage = async () => ({});
            bridge.oninitialized = async () => {
                console.log("[Prefab v2] Bridge initialized");
            };

            await bridge.connect(transport);

            bridge.oncalltool = async (params) => {
                console.log("[Prefab v2] CallTool:", params.name);
                const result = await client.callTool({
                    name: params.name,
                    arguments: params.arguments || {},
                });
            ...
            window._prefabRender = async function(resultsJson) {
                try {
                    const count = JSON.parse(resultsJson).length;
                    console.log("[Prefab v2] Rendering dashboard with", count, "results");
                    const result = await client.callTool({
                        name: "render_dashboard",
                        arguments: { results_json: resultsJson },
                    });
                    ...
                    console.warn("[Prefab v2] Render failed:", err.message);
                }
            };

            console.log("[Prefab v2] Ready");
        } catch (err) {
            console.warn("[Prefab v2] Init failed:", err.message);
        }
    </script>
</body>
</html>
